...
from ..data.tfms import normalize
...
parser.add_argument('--detections', default='cosy')
...
eval_name = f'{model_name}-{args.detections}'
...
data = detector_crops.DetectorCropDataset(
    ...
    detection_folder=Path(f'data/detection_results/{args.detections}/{dataset}'),
...
def infer(i, d):
    obj_idx = d['obj_idx']
    img = d['rgb_crop']  # (H, W, 3)
    K_crop = d['K_crop']
    mask = d['mask_visib_crop'] if model.use_instance_segmentation else None

    with utils.add_timing_to_list(time_forward):
        mask_lgts, query_img = model.infer_cnn(img, obj_idx, rotation_ensemble=args.rotation_ensemble, mask=mask)
    ...
    with utils.add_timing_to_list(time_refine):
        if success:
            R_est_r, t_est_r, score_r = refine_pose(
                world_R_obj=R_est, world_t_obj=t_est, query_imgs=query_img[None], K_crops=K_crop[None],
                cams_T_world=np.eye(4)[None],
