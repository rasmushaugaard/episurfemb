...
parser.add_argument('--train-auxs', action='store_true')
...
if args.train_auxs:
    auxs = model.get_auxs(objs=objs, crop_res=res_crop)[:-2]
else:
    auxs = model.get_infer_auxs(objs=objs, crop_res=res_crop, from_detections=bool(detection))
dataset_args = dict(dataset_root=root, obj_ids=obj_ids, auxs=auxs, cfg=cfg)
if detection:
    data = detector_crops.DetectorCropDataset(
        detection_folder=Path(f'data/detection_results/{detection}/{dataset}'),
...
print("press 'p' to toggle pre-multiplying queries with mask before refinement,")
print("press 'q' to quit.")
pre_mult_query_with_mask = False
...
while True:
    ...
    img = inst['rgb_crop']
    mask_visib = inst['mask_visib_crop'] if model.use_instance_segmentation else None
    if mask_visib is not None:
        cv2.imshow('mask_visib', mask_visib)
    K_crop = inst['K_crop']
    obj_ = objs[obj_idx]
    print(f'i: {data_i}, obj_id: {obj_ids[obj_idx]}')

    with utils.timer('forward_cnn'):
        mask_lgts, query_img = model.infer_cnn(img, obj_idx, mask=mask_visib)
        ...
        if key == ord('q'):
            ...
            if current_pose is not None:
                with utils.timer('refinement'):
                    query_img_ = query_img
                    if pre_mult_query_with_mask:
                        query_img_ = query_img_ * mask_prob[..., None]
                    R, t, score_r = pose_refine.refine_pose(
                        R=current_pose[0], t=current_pose[1], query_img=query_img_, keys_verts=keys_verts,
                        obj_idx=obj_idx, obj_=obj_, K_crop=K_crop, model=model, renderer=renderer,
                    )
                    trace = np.trace(R @ current_pose[0].T)
                    angle = np.arccos(np.clip((trace - 1) / 2, -1, 1))
                    print(f'ref. angle diff: {np.rad2deg(angle):.1f} deg')
                    print(f'ref. translation diff: {np.around(t - current_pose[1], 1).reshape(-1)}')
                debug_pose_hypothesis(R, t)
        elif key == ord('p'):
            pre_mult_query_with_mask = not pre_mult_query_with_mask
            print(f'premultiply queries with mask: {pre_mult_query_with_mask}')
