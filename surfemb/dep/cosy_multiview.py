...
def build_frame_index(base_dir: Path):
    ...
    for scene_dir in base_dir.iterdir():
        for view_id, view in json.load((scene_dir / 'scene_camera.json').open()).items():
            ...
            Ks.append(K)
    return pd.DataFrame(dict(scene_id=scene_ids, view_id=view_ids, cam_t_world=cam_t_worlds, K=Ks))


def get_multiview_frame_index(frame_index: pd.DataFrame, n_views: int):
    """modified version of cosypose/datasets/wrappers/multiview_wrapper.py:MultiViewWrapper:__init__"""
    ...
    for scene_id, group_ids in groups.items():
        ...
        for k, m in enumerate(np.arange(len_group)[::n_views]):
            ...
            multiview_frame_index.append(dict(
                scene_id=scene_id,
