...
class DetectorCropDataset(torch.utils.data.Dataset):
    def __init__(
            ...
            auxs: Sequence[BopInstanceAux], show_progressbar=True, load_masks=False,
    ):
        ...
        self.masks = np.load(str(detection_folder / 'masks.npy')) if load_masks else None
    ...
    def __getitem__(self, i):
        ...
        if self.masks is not None:
            instance['mask_visib_crop'] = self.masks[i]
        for aux in self.auxs:
...
def _main():
    ...
    dataset_root = Path(f'data/bop/{args.dataset}')
    ...
    data = DetectorCropDataset(
        ...
        detection_folder=Path(f'data/detection_results/{args.detections}/{args.dataset}'),
        auxs=(
            std_auxs.RgbLoader(),
