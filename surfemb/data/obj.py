...
def load_obj(models_root: Path, obj_id: int):
...
def load_objs(models_root: Path, obj_ids: Sequence[int] = None, show_progressbar=True):
    objs = []
    if obj_ids is None:
        obj_ids = sorted(map(int, json.load((models_root / 'models_info.json').open()).keys()))
