...
class ObjCoordRenderer:
    def __init__(self, objs: Sequence[Obj], w: int, h: int = None, device_idx=0):
        ...
        self.prog = self.ctx.program(
            vertex_shader="""
                ...
                void main() {
                    ...
                    color = flat_render ? vec3(-(rot * normal).z) : (in_vert - offset) / scale;
                }
                """,
            fragment_shader="""
                #version 330
        ...
        for obj in self.objs:
            vertices = obj.mesh.vertices[obj.mesh.faces].astype('f4')  # (n, 3, 3)
            normals = np.repeat(obj.mesh.face_normals, 3, axis=0).astype('f4')  # (n3, 3)
            vao = self.ctx.vertex_array(self.prog, [
                (self.ctx.buffer(vertices), '3f', 'in_vert'),
                (self.ctx.buffer(normals), '3f', 'normal'),
            ])
    ...
    def render(self, obj_idx, K, R, t, clear=True, read=True, read_depth=False, flat=False):
        ...
        self.prog['rot'].value = tuple(R.T.astype('f4').reshape(-1))
        self.prog['scale'].value = obj.scale
        self.prog['offset'].value = tuple(obj.offset.astype('f4'))
        self.prog['flat_render'].value = flat
