...
class SurfaceEmbeddingModel(pl.LightningModule):
    def __init__(self, n_objs: int, emb_dim=12, n_pos=1024, n_neg=1024, lr_cnn=3e-4, lr_mlp=3e-5,
                 ...
                 key_noise=1e-3, warmup_steps=2000, separate_decoders=True, bbox_from_hull=False,
                 one_cycle=False, max_steps: int = None, weight_decay=0., use_instance_segmentation=False,
                 p_random_fill=0.,
                 **kwargs):
        """
        ...
        self.p_random_fill = p_random_fill
        ...
        if use_instance_segmentation:
            self.cnn.base_model.conv1.weight.data = torch.cat((
                self.cnn.base_model.conv1.weight.data,
                torch.zeros(64, 1, 7, 7),
            ), dim=1)
            self.cnn.base_model.conv1.in_channels = 4
        # key models
    ...
    def model_specific_args(parent_parser: argparse.ArgumentParser):
        ...
        parser.add_argument('--bbox-from-hull', action='store_true')
        parser.add_argument('--one-cycle', action='store_true')
        parser.add_argument('--weight-decay', type=float, default=0.)
        parser.add_argument('--use-instance-segmentation', action='store_true')
        parser.add_argument('--p-random-fill', type=float, default=0.)
        return parent_parser

    def get_auxs(self, objs: Sequence[Obj], crop_res: int):
        random_crop_aux = data.std_auxs.RandomRotatedMaskCrop(crop_res, bbox_from_hull=self.bbox_from_hull)
        return (
            data.std_auxs.RgbLoader(),
            data.std_auxs.MaskLoader(),
            data.pose_auxs.RandomMaskFillAux(p=self.p_random_fill),
            data.pose_auxs.ConvexHullProjectionAux(objs),
            ...
            data.pose_auxs.ObjCoordAux(objs, crop_res),
            ...
            data.std_auxs.TransformsAux(tfms=data.tfms.NoisyMask(), key='mask_visib_crop'),
            data.std_auxs.NormalizeAux(),
            data.std_auxs.KeyFilterAux({'rgb_crop', 'obj_coord', 'mask_visib_crop',
                                        'obj_idx', 'surface_samples', 'mask_samples'})
        )
    ...
    def get_infer_auxs(self, objs: Sequence[Obj], crop_res: int, from_detections=True):
        ...
        if not from_detections:
            ...
            auxs.append(data.pose_auxs.ConvexHullProjectionAux(objs))
        auxs.append(data.std_auxs.RandomRotatedMaskCrop(
            ...
            bbox_from_hull=self.bbox_from_hull,
        ))
        if not from_detections:
            auxs += [
                data.pose_auxs.ObjCoordAux(objs, crop_res),
    ...
    def configure_optimizers(self):
        ...
        if self.one_cycle:
            sched = torch.optim.lr_scheduler.OneCycleLR(
                opt, max_lr=[self.lr_cnn, self.lr_mlp], total_steps=self.max_steps,
            )
        else:
            sched = torch.optim.lr_scheduler.LambdaLR(opt, lambda i: min(1., i / self.warmup_steps))
        sched = dict(scheduler=sched, interval='step')
        return [opt], [sched]

    def step(self, batch, log_prefix):
        img = batch['rgb_crop']  # (B, 3, H, W)
        if self.use_instance_segmentation:
            mask_visib = batch['mask_visib_crop']  # (B, H, W)
            img = torch.cat((img, mask_visib[:, None]), dim=1)  # (B, 4, H, W)
    ...
    def log_image_sample(self, batch, i=0):
        ...
        mask_visib = batch['mask_visib_crop'][i] if self.use_instance_segmentation else None

        mask_lgts, query_img = self.infer_cnn(img, obj_idx, mask=mask_visib)
    ...
    @torch.no_grad()
    def infer_cnn(self, img: Union[np.ndarray, torch.Tensor], obj_idx, rotation_ensemble=True, mask=None):
        assert not self.training
        if isinstance(img, np.ndarray):
            if img.dtype == np.uint8:
                img = data.tfms.normalize(img)
            img = torch.from_numpy(img)
        assert (mask is not None) == self.use_instance_segmentation
        if self.use_instance_segmentation:
            if isinstance(mask, np.ndarray):
                if mask.dtype == np.uint8:
                    mask = (mask / 255).astype(np.float32)
                mask = torch.from_numpy(mask)
            img = torch.cat((img, mask[None]), dim=0)  # (4, r, r)
        img = img.to(self.device)
        _, h, w = img.shape

        if rotation_ensemble:
            img = utils.rotate_batch(img)  # (4, 3, r, r)
        else:
            img = img[None]  # (1, 3, r, r)
